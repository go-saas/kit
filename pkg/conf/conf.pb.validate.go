...
func (m *Data) validate(all bool) error {
	...
	{
		sorted_keys := make([]string, len(m.GetBlobs()))
		i := 0
		for key := range m.GetBlobs() {
			sorted_keys[i] = key
			i++
		}
		sort.Slice(sorted_keys, func(i, j int) bool { return sorted_keys[i] < sorted_keys[j] })
		for _, key := range sorted_keys {
			...
			// no validation rules for Blobs[key]
...
// Validate checks the field values on Email_SMTP with the rules defined in the
// proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *Email_SMTP) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on Email_SMTP with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in Email_SMTPMultiError, or
// nil if none found.
func (m *Email_SMTP) ValidateAll() error {
	return m.validate(true)
}

func (m *Email_SMTP) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if m.ConnectTimeout != nil {
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, Email_SMTPValidationError{
			...
			if err := v.Validate(); err != nil {
				return Email_SMTPValidationError{
					field:  "ConnectTimeout",
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if len(errors) > 0 {
		return Email_SMTPMultiError(errors)
	}

	return nil
}

// Email_SMTPMultiError is an error wrapping multiple validation errors
// returned by Email_SMTP.ValidateAll() if the designated constraints aren't met.
type Email_SMTPMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m Email_SMTPMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m Email_SMTPMultiError) AllErrors() []error { return m }

// Email_SMTPValidationError is the validation error returned by
// Email_SMTP.Validate if the designated constraints aren't met.
type Email_SMTPValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e Email_SMTPValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e Email_SMTPValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e Email_SMTPValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e Email_SMTPValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e Email_SMTPValidationError) ErrorName() string { return "Email_SMTPValidationError" }

// Error satisfies the builtin error interface
func (e Email_SMTPValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = Email_SMTPValidationError{}
