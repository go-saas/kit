...
func InitApp(services *conf.Services, security *conf.Security, userConf *conf2.UserConf, confData *conf.Data, logger log.Logger, webMultiTenancyOption *http.WebMultiTenancyOption, arg ...grpc.ClientOption) (*kratos.App, func(), error) {
	...
	dataData, cleanup3, err := data.NewData(confData, constDbProvider, logger)
	if err != nil {
		cleanup2()
		cleanup()
		return nil, nil, err
	...
	userRoleContrib := service.NewUserRoleContrib(userManager)
	...
	barrierMigrator := data2.NewBarrierMigrator(constDbProvider, connName)
	...
	seeding := server.NewSeeding(manager, migrate, barrierMigrator, roleSeed, userSeed, permissionSeeder)
