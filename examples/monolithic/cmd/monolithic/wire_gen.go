...
import (
	...
	data4 "github.com/go-saas/kit/dtm/data"
	service3 "github.com/go-saas/kit/dtm/service"
	...
	service4 "github.com/go-saas/kit/sys/private/service"
...
func initApp(services *conf.Services, security *conf.Security, confData *conf.Data, saasConf *conf2.SaasConf, userConf *conf3.UserConf, logger log.Logger, appConfig *conf.AppConfig, arg ...grpc.ClientOption) (*kratos.App, func(), error) {
	...
	data5, cleanup4, err := data2.NewData(confData, dbProvider, logger)
	if err != nil {
		cleanup3()
		cleanup2()
		cleanup()
	...
	userRepo := data2.NewUserRepo(data5)
	...
	roleRepo := data2.NewRoleRepo(data5, eventBus)
	...
	msgServiceService := service3.NewMsgService(constDbProvider, connName)
	httpServerRegister := service3.NewHttpServerRegister(msgServiceService)
	...
	userSettingRepo := data2.NewUserSettingRepo(data5, eventBus)
	userAddressRepo := data2.NewUserAddrRepo(data5, eventBus)
	...
	serviceHttpServerRegister := service2.NewHttpServerRegister(userService, encodeResponseFunc, encodeErrorFunc, accountService, authService, roleService, servicePermissionService, auth, confData, defaultAuthorizationService, factory)
	menuRepo := data3.NewMenuRepo(dbProvider, eventBus)
	menuService := service4.NewMenuService(defaultAuthorizationService, menuRepo, logger)
	redisConnOpt := job.NewAsynqClientOpt(client)
	httpServerRegister2 := service4.NewHttpServerRegister(menuService, defaultAuthorizationService, encodeErrorFunc, factory, confData, redisConnOpt)
	httpServerRegister3 := service.NewHttpServerRegister(tenantService, factory, defaultAuthorizationService, encodeErrorFunc, tenantInternalService, confData)
	serverHttpServerRegister := server2.NewHttpServiceRegister(httpServerRegister, serviceHttpServerRegister, httpServerRegister2, httpServerRegister3)
	...
	grpcServerRegister := service3.NewGrpcServerRegister(msgServiceService)
	serviceGrpcServerRegister := service2.NewGrpcServerRegister(userService, accountService, authService, roleService, servicePermissionService)
	grpcServerRegister2 := service4.NewGrpcServerRegister(menuService)
	grpcServerRegister3 := service.NewGrpcServerRegister(tenantService, tenantInternalService)
	serverGrpcServerRegister := server2.NewGrpcServiceRegister(grpcServerRegister, serviceGrpcServerRegister, grpcServerRegister2, grpcServerRegister3)
