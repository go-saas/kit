...
func initApp(services *conf.Services, security *conf.Security, confData *conf.Data, saasConf *conf2.SaasConf, userConf *conf3.UserConf, logger log.Logger, appConfig *conf.AppConfig, arg ...grpc.ClientOption) (*kratos.App, func(), error) {
	...
	userTenantContributor := api2.NewUserTenantContributor(userService)
	...
	refreshTokenProvider := api2.NewRefreshProvider(authService, logger)
	...
	accountService := service.NewAccountService(userManager, factory, tenantService, userSettingRepo, userAddressRepo, lookupNormalizer)
