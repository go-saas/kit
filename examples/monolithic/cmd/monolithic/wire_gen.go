...
import (
	...
	service4 "github.com/go-saas/kit/dtm/service"
...
func initApp(services *conf.Services, security *conf.Security, confData *conf.Data, saasConf *conf2.SaasConf, userConf *conf3.UserConf, logger log.Logger, appConfig *conf.AppConfig, arg ...grpc.ClientOption) (*kratos.App, func(), error) {
	...
	userRoleContrib := service2.NewUserRoleContrib(userManager)
	...
	serviceHttpServerRegister := service4.NewHttpServerRegister(msgService)
	...
	httpServerRegister2 := service2.NewHttpServerRegister(userService, encodeResponseFunc, encodeErrorFunc, accountService, authService, roleService, servicePermissionService, auth, confData, defaultAuthorizationService, factory)
	menuRepo := data3.NewMenuRepo(dbProvider, eventBus)
	menuService := service5.NewMenuService(defaultAuthorizationService, menuRepo, logger)
	httpServerRegister3 := service5.NewHttpServerRegister(menuService, defaultAuthorizationService, encodeErrorFunc, factory, confData, redisConnOpt)
	httpServerRegister4 := service.NewHttpServerRegister(tenantService, factory, defaultAuthorizationService, encodeErrorFunc, tenantInternalService, confData)
	serverHttpServerRegister := server2.NewHttpServiceRegister(httpServerRegister, serviceHttpServerRegister, httpServerRegister2, httpServerRegister3, httpServerRegister4)
	...
	grpcServerRegister := service3.NewGrpcServerRegister(eventService)
	serviceGrpcServerRegister := service4.NewGrpcServerRegister(msgService)
	grpcServerRegister2 := service2.NewGrpcServerRegister(userService, accountService, authService, roleService, servicePermissionService)
	grpcServerRegister3 := service5.NewGrpcServerRegister(menuService)
